...
import { deleteByIds, page } from '../../api/jileappCmsArticle.ts'
...
defineOptions({ name: 'article:jileappCmsArticle' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('cms.menu.article'),
    subTitle: () => t('cms.article.subTitle'),
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['article:jileappCmsArticle:save']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['article:jileappCmsArticle:delete']"
          type="danger"
          plain
