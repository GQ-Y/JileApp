...
import { deleteByIds, page } from '../../api/jileappCmsSnippets.ts'
...
defineOptions({ name: 'snippets:jileappCmsSnippets' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('cms.menu.snippets'),
    subTitle: () => t('cms.snippets.subTitle'),
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['snippets:jileappCmsSnippets:save']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['snippets:jileappCmsSnippets:delete']"
          type="danger"
          plain
