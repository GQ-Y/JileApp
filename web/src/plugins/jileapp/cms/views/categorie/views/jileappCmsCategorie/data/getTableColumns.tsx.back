...
import { deleteByIds } from '../../../api/jileappCmsCategorie.ts'
import { jileappCmsCategorieStatusDictData } from './common.tsx'
...
export default function getTableColumns(dialog: UseDialogExpose, formRef: any, t: any): MaProTableColumns[] {
  ...
  return [
    ...
    { label: t('cms.categorie.id'), prop: 'id' },
    ...
    { label: t('cms.categorie.sort'), prop: 'sort' },
    {
      label: t('cms.categorie.status'), prop: 'status_switch', cellRender: ({ row }) => {
        return (
          <ElTag type='primary'>
            {jileappCmsCategorieStatusDictData(t).find((item: any) => item.value === row.status_switch)?.label}
          </ElTag>
        )
      },
    },
