...
import { deleteByIds, page } from '../../api/jileappCmsAdPosition.ts'
...
defineOptions({ name: 'position:jileappCmsAdPosition' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('cms.menu.position'),
    subTitle: () => t('cms.position.subTitle'),
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['position:jileappCmsAdPosition:save']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['position:jileappCmsAdPosition:delete']"
          type="danger"
          plain
