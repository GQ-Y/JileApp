...
/**
* 添加JileappCmsAdPosition
* @returns
*/
export function create(data: any): Promise<ResponseStruct<any>> {
  return useHttp().post('position/jileappCmsAdPosition', data)
}
/**
* 更新JileappCmsAdPosition数据
* @returns
*/
export function save(id: number, data: any): Promise<ResponseStruct<any>> {
    return useHttp().put(`position/jileappCmsAdPosition/${id}`, data)
}

/**
* 将JileappCmsAdPosition删除，有软删除则移动到回收站
* @returns
*/
export function deleteByIds(ids: number[]): Promise<ResponseStruct<null>> {
  return useHttp().delete('position/jileappCmsAdPosition', { data: ids })
}